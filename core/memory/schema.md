...
├── friction.md          # Friction conventions (loaded at session start)
├── decisions/           # Architecture Decision Records
│   ├── ADR-001-*.md
│   └── ADR-NNN-*.md
│   └── INDEX.md          # Auto-generated decision lookup table
├── corrections-queue.md  # Ephemeral correction/friction queue (auto-populated)
├── .last-sync            # Unix epoch timestamp of last /memory-sync (cross-session diffing)
...
Use `/memory-search <query>` to find entries across memory files. Supports `--global` for global memory, `--cross-project` for cross-project search, and `--sessions` to include session files.
...
**Purpose:** Auto-generated lookup table of all Architecture Decision Records. Generated by `/memory-sync` (Step 4.6). Agents are instructed to use this as a lookup table rather than reading every ADR file.

**Generation:** Agent-executed during `/memory-sync`. Parses ADR headers, metadata, and supersession information. Skips ADRs with `private: true` frontmatter. Parse errors are included with a fallback title.

**Agent behavior:** A directive in the session-start context instructs agents to use INDEX.md as a lookup table. This is always present (~20 tokens), not gated by token budget.
...
**Purpose:** Session retrospective generated by `/memory-reflect`. Contains improvement recommendations, approval status, and evidence summary. Not loaded at session start — consumed on-demand.

**Required sections:** Session Summary, Improvement Log (Approved/Declined), Improvement Backlog, Evidence.
See `/memory-reflect` skill (Phase 6) for the full template format.

**Retrospective categories:** Recommendations use the retrospective category set: `efficiency`, `quality`, `ux`, `knowledge`, `architecture`. See **Retrospective Categories** in the Category Tags section below.

**Lightweight variant:** Sessions with < 5 observations and 0 corrections produce a minimal retro with only Summary and Evidence sections.

### friction.md

**Purpose:** Friction-reducing conventions discovered by `/memory-insights` and loaded at session start. Helps agents avoid known pitfalls and repeat mistakes.

**Generation:** Created by `/memory-init` (empty template). Populated by `/memory-insights` when routing analysis findings. Editable by users.

**Loading:** Loaded by `session-start.sh` into context, gated by token budget:

| Preset | friction.md behavior |
|---|---|
| Economy (~2000) | Not loaded |
| Light (~3000) | Loaded, capped at ~200 tokens (~800 chars) |
| Standard (~4000) | Loaded, capped at ~400 tokens (~1600 chars) |
| Detailed (~6000) | Loaded, full content |

**Cross-referencing:** `/memory-reflect` reads friction.md during Phase 3 (Analyze Patterns) to check if known friction patterns were triggered or avoided.

**Entry cap:** ~15-20 entries maximum. When `/memory-insights` regenerates, it keeps top entries by recurrence frequency, drops the rest.

**Security:** Refused if file is a symlink.
...
**Purpose:** Cross-session sync marker. Records when `/memory-sync` last ran, enabling the session-start hook to show what changed since the last sync (memory diff) and to assess memory staleness (health scoring).
...
**Readers:** `session-start.sh` — reads epoch from file content (not mtime, which is fragile to backup tools and file sync).

**Security:** Refuse to read or write through symlinks. If the file is a symlink, skip silently.

**First-run behavior:** If `.last-sync` does not exist, the session-start hook creates it with `date +%s` and skips both health scoring and diff for that session (avoids false "everything stale" on first use or upgrade from v1.1.0).

**Multi-agent note:** Multiple agents sharing a project overwrite `.last-sync` on each sync. Agent B sees diff relative to Agent A's last sync. Per-agent markers are deferred to v1.3.0.

### Health Scoring

**Purpose:** At session start, the hook counts commits since each memory file's mtime. If the count exceeds `staleness_commits` (default 5), the file is flagged as stale with a directive to run `/memory-sync`.

**Known limitations:**
- Commit counting with mtime is fragile across rebases/force-pushes (hashes change, mtimes don't). Acceptable for advisory-only feature.
- Commit-only metric (no calendar component). A hybrid metric (commits + days) is a candidate for v1.3.0.
- Performance guard: if health check takes >1s, partial results are cached and remaining files are skipped.

**Cache:** Results are cached to `$TMPDIR/conkeeper/health-<YYYYMMDD>` so the stop hook reads cached results without re-running git.
...
staleness_commits: 5            # Commits since last file update before flagging stale; 0 = disable
